...
5724-H72 
This package provides an IBM Java 8 Runtime Environment for running the Java
...
install -d $RPM_BUILD_ROOT/opt/mqm/java/jre64
install /build/slot2/p900_P/obj/amd64_linux_2/ship/opt/mqm/swidtag/IBM_MQ_JRE-1.8.0.mqtag $RPM_BUILD_ROOT/opt/mqm/swidtag/IBM_MQ_JRE-1.8.0.mqtag
install /build/slot2/p900_P/obj/amd64_linux_2/ship/opt/mqm/java/jre64/mq_jre64.tar.gz $RPM_BUILD_ROOT/opt/mqm/java/jre64/mq_jre64.tar.gz

%files
%dir %attr(555,mqm,mqm) "/opt/mqm"
%dir %attr(555,mqm,mqm) "/opt/mqm/swidtag"
%dir %attr(555,mqm,mqm) "/opt/mqm/java"
%dir %attr(555,mqm,mqm) "/opt/mqm/java/jre64"
%attr(444,mqm,mqm) "/opt/mqm/swidtag/IBM_MQ_JRE-1.8.0.mqtag"
%attr(444,mqm,mqm) "/opt/mqm/java/jre64/mq_jre64.tar.gz"

%pre
RPM_PACKAGE_SUMMARY="IBM MQ Java JRE"
RPM_PACKAGE_NAME="MQSeriesJRE"
...
# @(#) MQMBID sn=p900-L160520.DE su=_6zq1EB58Eearh6Qyg9d9Dg pn=install/unix/linux_2/jre_postinstall.sh
#   <copyright 
#   notice="lm-source-program" 
#   pids="5724-H72," 
#   years="2004,2012" 
...
#   </copyright> 
#
#   post install script for the MQSeriesJRE package
TARFLAGS='-xzf'
echo ${SHELLOPTS} | grep xtrace > /dev/null
if [ $? -eq 0 ] ; then
  TARFLAGS='-xvzf'
...
for JRE in jre jre64 
do 
  JAVA_HOME=`echo ${MQ_INSTALLATION_PATH}/java/${JRE} | sed s#//#/#g`
  if [ -r ${JAVA_HOME}/mq_${JRE}.tar.gz ]  
  then 
    cd ${JAVA_HOME}
    tar ${TARFLAGS} mq_${JRE}.tar.gz 
    tarrc=$?
    if [ ${tarrc} -ne 0 ]
      then 
        echo "WARNING: Return code \"${tarrc}\" from tar while expanding \"${MQ_INSTALLATION_PATH}/java/${JRE}/mq_${JRE}.tar.gz\""
      else 
        chown -h -R mqm:mqm .
        chmod -R o-w,g-w,u-w .
    fi
...
# Interim compatability symlink for Explorer
if [ -d ${MQ_INSTALLATION_PATH}/java/jre/jre/bin ] 
  then 
...
# @(#) MQMBID sn=p900-L160520.DE su=_6zq1EB58Eearh6Qyg9d9Dg pn=install/unix/linux_2/jre_preuninstall.sh
#   <copyright 
#   notice="lm-source-program" 
#   pids="5724-H72," 
#   years="2004,2012" 
...
do 
  ...
  then 
    cd ${JAVA_HOME}
    tar ${TARFLAGS} mq_${JRE}.tar.gz | awk '{print $1}' | xargs rm -rf 
  fi
done 

if [ -h ${MQ_INSTALLATION_PATH}/java/jre/bin ] 
  then 
   rm  ${MQ_INSTALLATION_PATH}/java/jre/bin 
fi
